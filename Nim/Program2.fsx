...
let easyButton = new Button(Location = Point(50, 65), Size = Size(100, 50), Text = "EASY")
let hardButton = new Button(Location = Point(200, 65), Size = Size(100, 50), Text = "HARD")
let clearButton = new Button(Location = Point(350, 65), Size = Size(100, 50), Text = "CLEAR")
let endTurnButton = new Button(Location = Point(750, 65), Size = Size(100, 50), Text = "END TURN")
let cancelButton = new Button(Location = Point(500, 65), Size = Size(100, 50), Text = "ABORT")
...
let combo = 
    new ComboBox(Location = Point(100, 35), Width = 300, 
                 DataSource = [| "http://www2.compute.dtu.dk/~mire/02257/nim1.game"; 
                                 "http://www2.compute.dtu.dk/~mire/02257/nim2.game"; 
                                 "http://www2.compute.dtu.dk/~mire/02257/nim3.game"; 
                                 "http://www2.compute.dtu.dk/~mire/02257/nim4.game"; "Use url in box above" |])
let ans = new TextBox(Location = Point(125, 10), Text = "http://", Width = 250)
    ...
    Seq.toArray (seq { 
                     for i in 1..arr.Length do
                         for x in 1..arr.[i - 1] do
                             yield new PictureBox(Image = Image.FromFile("hatteland2.png"), Top = (i * 50 + 75), 
                                                  Left = (350 - (x * 10)), Width = 5)
                 }
                 |> Seq.cast<Control>)
...
let guifucker butt matt = 
    Seq.iter (fun x -> window.Controls.Remove x) butt
    Seq.iter (fun x -> window.Controls.Remove x) matt

let rec ready() = 
    async { 
        guifucker buttons matches
    ...
    async { 
        ...
        Async.StartWithContinuations
            (async { 
                 ...
                 let! html = webCl.AsyncDownloadString(Uri(if url = "Use url in box above" then ans.Text
                                                           else url))
                 return html
        ...
        | Web html -> 
            let arr = 
                Array.map (fun x -> 
                    if x <= 9 && x > 0 then x
                    else 9) [| for x in Regex("\d+").Matches(html) -> int x.Value |]
            //let arr = [|1;2;3;4|]
...
and player (arr, t) = 
    async { 
        ...
        if t then endTurnButton.Enabled <- true
        updateBoard arr
        if checkGameState arr then return! finished ("lose")
        let! msg = ev.Receive()
        match msg with
...
and turn (arr, i) = 
    async { 
        disable [ easyButton; hardButton; clearButton; cancelButton ]
...
and ai (arr) = 
    async { 
        ...
        updateBoard arr
        let newArr = 
            if optimal then getOptimal arr
            else getRandom (rnd.Next(0, arr.Length), arr)
        for a in 0..buttons.Length - 1 do
            if newArr.[a] <> 0 then 
